...
class CRF(nn.Module):
    ...
    def decode(self,
               ...
               mask: Optional[Union[Tensor, ByteTensor]] = None,
               prob: bool = False,
               n_best: int = 1,
               ) -> List[List[int]]:
...
#         mask = mask.data
        ...
        if prob:
            return self._viterbi_decode_with_prob(emissions, mask, n_best)
        else:
    ...
    def _viterbi_decode(self, emissions: FloatTensor, mask: ByteTensor) \
        ...
        for idx in range(batch_size):
            ...
            _, best_last_tag = (viterbi_score[seq_end][idx] + self.end_transitions.data).max(0)  # expected scalar value
    ...
    def _backward_a_star(self, emissions: Union[Tensor, FloatTensor],
                         viterbi_score_list: Union[Tensor, FloatTensor],
                         sequence_lengths: int, n_best: int) -> List[List[int]]:

        def get_viterbi_score(viterbi_score_list, curr_n):
            if curr_n:
                return viterbi_score_list[curr_n - 1].squeeze() * -1
            else:
                return torch.zeros_like(viterbi_score_list[0]).squeeze()

        def get_edge_score(prev_node):
            if prev_node > -float('inf'):
                return (self.transitions[:, prev_node].squeeze()) * -1
            else:
                return self.end_transitions * -1

        def get_node_score(prev_node, curr_n):
            if prev_node > -float('inf'):
                return emissions[curr_n].squeeze()[prev_node] * -1
            else:
        ...
        heapq.heappush(hq, (0, next(counter), {'g_score': torch.tensor(0), 'viterbi_path': [-float('inf')]}))

        while True:
            _, _, node = heapq.heappop(hq)

            curr_n = (sequence_lengths - len(node['viterbi_path'])) + 1

            if curr_n == 0:
                path = node['viterbi_path'][1:]
                path.reverse()
                n_best_path.append(path)
                if len(n_best_path) == n_best:
                    break
            else:
                prev_node = node['viterbi_path'][-1]
                g_score_list = node['g_score'] + get_edge_score(prev_node) + get_node_score(prev_node, curr_n)
                f_score_list = get_viterbi_score(viterbi_score_list, curr_n)
                current_path = node['viterbi_path']

                for prev_node, (g_score, f_score) in enumerate(zip(g_score_list, f_score_list)):
                    viterbi_path = copy.deepcopy(current_path)
                    viterbi_path.append(prev_node)
                    heapq.heappush(hq, (g_score + f_score, next(counter), {'g_score': g_score,
                                                                           'viterbi_path': viterbi_path}))

        return n_best_path

    def _viterbi_decode_with_prob(self, emissions: FloatTensor,
                                  mask: ByteTensor, n_best: int = 1) \
            -> List[List[int]]:
        # Get input sizes
        seq_length = emissions.size(0)
        batch_size = emissions.size(1)
        ...
        for i in range(1, seq_length):
            ...
            # Find the maximum score over all possible current tag
            best_score, best_path = score.max(1)  # (batch_size,num_tags,)
            # get probablity of current tag by next tag
            prob = F.softmax(score, dim=1)
            prob_list.append(prob)
            # Find the maximum probablity over all possible current tag
            best_prob, _ = prob.max(1)  # (batch_size,num_tags,)
            # Save the score and probablity and the path
            viterbi_score.append(best_score)
            viterbi_prob.append(best_prob)
            viterbi_all_prob.append(prob)
            viterbi_path.append(best_path)

        n_best_path_list = self._backward_a_star(emissions, viterbi_score, sequence_lengths, n_best=n_best)
        n_best_score_list = []
        n_best_prob_list = []
        for n_best_path in n_best_path_list:
            i = 0
            n_best_score = []
            n_best_prob = []
            while i < (sequence_lengths - 1):
                n_best_score.append(score_list[i][0][n_best_path[i]][n_best_path[i+1]].item())
                n_best_prob.append(prob_list[i][0][n_best_path[i]][n_best_path[i+1]].item())
                i += 1
            last_score = viterbi_score[seq_length - 1][0] + self.end_transitions.data
            n_best_score.append(last_score[n_best_path[-1]].item())
            n_best_score_list.append(n_best_score)
            last_prob = F.softmax(last_score, dim=0)
            n_best_prob.append(last_prob[n_best_path[-1]].item())
            n_best_prob_list.append(n_best_prob)
        n_best_list = [(n_best_path,n_best_prob, n_best_score) for n_best_path, n_best_prob, n_best_score in zip(n_best_path_list, n_best_prob_list, n_best_score_list)]
        ...
        #     # for the last timestep
        #     seq_end = sequence_lengths[idx]-1
        #     _, best_last_tag = (viterbi_score[seq_end][idx] + self.end_transitions.data).max(0)  # scalar
        #     last_prob = F.softmax(viterbi_score[seq_end][idx] + self.end_transitions.data, dim=0)  # [num_tags]
        #     best_last_prob, _ = last_prob.max(0)  # scalar
        #     best_tags = [best_last_tag.item()]  # [scalar]
        #     best_probs = [best_last_prob.item()]  # [scalar]
        #     best_all_probs = [last_prob.tolist()]  # [[num_tags]]
        #
        #     # We trace back where the best last tag comes from, append that to our best tag
        #     # sequence, and trace it back again, and so on
        #     for curr_seq, path in enumerate(reversed(viterbi_path[:sequence_lengths[idx] - 1])):
        #         best_last_tag = path[idx][best_tags[-1]]
        #         best_last_prob = viterbi_prob[seq_end - curr_seq - 1][idx][best_tags[-1]]
        #         best_last_all_prob = viterbi_all_prob[seq_end - curr_seq - 1][idx][:,best_tags[-1]]
        #         best_tags.append(best_last_tag.item())
        #         best_probs.append(best_last_prob.tolist())
        #         best_all_probs.append(best_last_all_prob.tolist())
        #
        #     # Reverse the order because we start from the last timestep
        #     best_tags.reverse()
        #     best_probs.reverse()
        #     best_all_probs.reverse()
        #     best_tags_list.append(best_tags)
        #     best_probs_list.append(best_probs)
        #     best_all_probs_list.append(best_all_probs)
